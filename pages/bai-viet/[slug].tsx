...
type PostData = {
  ...
  relatedPosts: {
    ...
    category: string;
    thumbnail: string;
    createdAt: string; // Add createdAt for related posts
  }[];
};
...
const host = process.env.NEXT_PUBLIC_HOST || "https://greenlahome.vn"; // Use env variable
const defaultImage = `${host}/images/noi-that-1.jpg`;

export const APP_NAME = "Bài viết Greenla Home";
          ...
          <div className="pt-5">
            ...
            <div className="flex flex-col space-y-4">
              {relatedPosts.slice(0, 5).map((p) => (
                <Link key={p.slug} href={`/bai-viet/${p.slug}`} className="flex space-x-3 items-start group">
                  {p.thumbnail && (
                    <Image
                      src={p.thumbnail}
                      alt={p.title}
...
> = async ({ params }) => {
  try {
    ...
    const relatedPosts = posts.map((p) => ({
      ...
      thumbnail: p.thumbnail?.url || defaultImage,
      createdAt: p.createdAt.toString(), // Include createdAt
    }));

    const { _id, title, content, meta, slug, tags, thumbnail, category, createdAt } = post;

    // Truncate description for SEO
    const truncatedDescription = trimText(meta, 160);
    ...
    const postData: PostData = {
      ...
      thumbnail: thumbnail?.url || defaultImage,
